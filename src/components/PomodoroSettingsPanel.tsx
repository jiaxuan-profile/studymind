// src/components/PomodoroSettingsPanel.tsx
...
const PomodoroSettingsPanel: React.FC = () => {
  ...
  type SettingKey = keyof PomodoroSettings;

  const updateSetting = (key: SettingKey, value: number | boolean) => {
    updatePomodoroSettings({ [key]: value });
  };

  const settingsConfig = [
    { label: 'Work Duration (minutes)', key: 'workDuration' as SettingKey, max: 120, min: 1 },
    { label: 'Short Break (minutes)', key: 'shortBreakDuration' as SettingKey, max: 30, min: 1 },
    { label: 'Long Break (minutes)', key: 'longBreakDuration' as SettingKey, max: 60, min: 1 },
    { label: 'Cycles before Long Break', key: 'cyclesBeforeLongBreak' as SettingKey, max: 10, min: 2 }
  ];
