...
const ExamDatesList: React.FC<ExamDatesListProps> = ({ onEditExamDate, onExamDateDeleted, onAddExamDate }) => {
  ...
  const handleDeleteConfirm = async () => {
    ...
    try {
      ...
      setExamDates(prev => prev.filter(date => date.id !== deletingId));
      onExamDateDeleted(); // Notify parent to refresh if needed
    } catch (error: any) {
              ...
              <div className="flex items-center justify-between">
                <div>
                  ...
                  <p className="text-sm text-gray-600 dark:text-gray-400">
                    <CalendarDays className="inline-block h-4 w-4 mr-1 text-primary" />
                    ...
                    {exam.subject_id && (
                      <span className="ml-3 px-2 py-0.5 bg-blue-100 dark:bg-blue-900/30 text-blue-800 dark:text-blue-300 text-xs rounded-full">
                        Subject ID: {exam.subject_id} {/* TODO: Display subject name */}
                      </span>
                    )}
                  </p>
                  {exam.notes && (
                    <p className="text-sm text-gray-700 dark:text-gray-300 mt-2">{exam.notes}</p>
                  )}
                </div>
                <div className="flex items-center space-x-2">
                  <button
                    onClick={() => onEditExamDate(exam)}
