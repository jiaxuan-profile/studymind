...
import { 
  ...
  Edit3, History, Clock, List, MessageSquare, FileQuestion, Crown
} from 'lucide-react';
...
const ReviewPage: React.FC = () => {
  ...
  const [selectedQuestionGeneration, setSelectedQuestionGeneration] = useState<'existing' | 'new'>('existing');
  const [selectedNewQuestionDifficulty, setSelectedNewQuestionDifficulty] = useState<'easy' | 'medium' | 'hard' | 'custom'>('custom');
  ...
  const canGenerateNewQuestions = canGenerateQuestions(userProfile?.subscription_tier || 'standard');
  ...
  // Calculate total questions based on current selection
  ...
  const resetReview = () => {
    ...
    setSelectedQuestionGeneration('existing');
    setSelectedNewQuestionDifficulty('custom');
    setCurrentQuestions([]);
    setCurrentQuestionIndex(0);
    setShowHint(false);
    setReviewedCount(0);
          ...
          <div className="space-y-6">
            <div className="bg-white rounded-xl shadow-lg border border-gray-200 overflow-hidden">
              ...
              <div className="p-6 space-y-6">
                {/* Question Generation Type (Pro Only) */}
                {isProUser && canGenerateNewQuestions && (
                  <div>
                    <h4 className="text-sm font-semibold text-gray-700 mb-3">Question Generation</h4>
                    <div className="space-y-3">
                      <button
                        onClick={() => setSelectedQuestionGeneration('existing')}
                        className={`w-full p-3 rounded-lg border-2 text-left transition-all ${
                          selectedQuestionGeneration === 'existing'
                            ? 'border-primary bg-primary/5'
                            : 'border-gray-200 hover:border-gray-300 hover:bg-gray-50'
                        }`}
                      >
                        <div className="flex items-center justify-between">
                          <div className="flex items-center">
                            <BookOpen className="h-4 w-4 mr-2" />
                            <span className="font-medium">Use Existing Questions</span>
                          </div>
                        ...
                        <p className="text-sm text-gray-600 mt-1">
                          Use questions generated when you uploaded the documents
                        </p>
                      </button>

                      <button
                        onClick={() => setSelectedQuestionGeneration('new')}
                        className={`w-full p-3 rounded-lg border-2 text-left transition-all ${
                          selectedQuestionGeneration === 'new'
                            ? 'border-primary bg-primary/5'
                            : 'border-gray-200 hover:border-gray-300 hover:bg-gray-50'
                        }`}
                      >
                        <div className="flex items-center justify-between">
                          <div className="flex items-center">
                            ...
                            <span className="font-medium">Generate New Questions</span>
                        ...
                        <p className="text-sm text-gray-600 mt-1">
                          Generate fresh questions tailored to your current mastery level
                        </p>
                      </button>
                    </div>
                  </div>
                ...
                <div>
                  <h4 className="text-sm font-semibold text-gray-700 mb-3">
                    {selectedQuestionGeneration === 'new' && isProUser ? 'New Question Difficulty' : 'Question Difficulty'}
                  </h4>
                  <div className="space-y-3">
                    {selectedQuestionGeneration === 'new' && isProUser ? (
                      // New question difficulty options for Pro users
                      <>
                        {(['easy', 'medium', 'hard', 'custom'] as const).map((difficulty) => (
                          <button
                            key={difficulty}
                            onClick={() => setSelectedNewQuestionDifficulty(difficulty)}
                            className={`w-full p-3 rounded-lg border-2 text-left transition-all ${
                              selectedNewQuestionDifficulty === difficulty
                                ? 'border-primary bg-primary/5'
                                : 'border-gray-200 hover:border-gray-300 hover:bg-gray-50'
                            }`}
                          >
                            <div className="flex items-center justify-between">
                              <div className="flex items-center">
                                ...
                                {difficulty === 'custom' && <Brain className="h-4 w-4" />}
                                <span className={`font-medium ${difficulty !== 'custom' ? 'ml-2' : 'ml-2'} capitalize`}>
                                  {difficulty === 'custom' ? 'Custom (Based on Mastery)' : difficulty}
                                </span>
                              </div>
                              <div className={`w-4 h-4 rounded-full border-2 ${
                                selectedNewQuestionDifficulty === difficulty
                            ...
                            <p className="text-sm text-gray-600 mt-1">
                              {difficulty === 'easy' && 'Generate easier questions for review'}
                              {difficulty === 'medium' && 'Generate moderately challenging questions'}
                              {difficulty === 'hard' && 'Generate advanced questions for mastery'}
                              {difficulty === 'custom' && 'AI selects difficulty based on your concept mastery levels'}
                            </p>
                          </button>
                        ))}
                      </>
                    ) : (
                      // Existing question difficulty filter
                      <>
              ...
              <div className="p-6">
                ...
                {selectedNotes.length > 0 ? (
                  <div className="space-y-3">
                    ...
                    {selectedQuestionGeneration === 'new' && isProUser && (
                      <div className="text-center">
                        <div className="inline-flex items-center px-3 py-1 rounded-full text-sm font-medium border border-yellow-200 bg-yellow-50 text-yellow-700">
                          <Crown className="h-4 w-4 mr-1" />
                          <span className="capitalize">
                            {selectedNewQuestionDifficulty === 'custom' ? 'Custom Difficulty' : `${selectedNewQuestionDifficulty} Questions`}
                          </span>
                        </div>
                      </div>
                    )}
