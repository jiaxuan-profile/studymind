...
const ReviewPage: React.FC = () => {
  ...
  useEffect(() => {
    ...
    if (user && user.id && subjects.length === 0 && !state?.retrySessionId && !isRetryingSession && !isLoadingSession) {
      loadSubjects().catch((error: any) => {
        console.error("AssessmentPage: Failed to load subjects:", error);
  ...
  const generateQuestions = async () => {
    ...
    addNotification('Starting question generation for selected notes', 'info', 'Assessment');
    ...
    try {
      ...
      addNotification('Question generation completed', 'success', 'Assessment');

    } catch (error) {
      console.error('Error generating questions:', error);
      addToast('Failed to generate questions', 'error');
      addNotification('Question generation failed', 'error', 'Assessment');
    } finally {
    ...
    return (
      <>
        ...
        <Dialog
          ...
          message={`You have an unfinished assessment session "${inProgressSession?.session_name || 'Untitled Session'}" from ${inProgressSession ? new Date(inProgressSession.started_at).toLocaleString() : 'recently'}. Would you like to resume it or start a new session?`} 
  ...
  if (currentStep === 'review') {
    if (isLoadingSession && !currentQuestions.length) {
      return <div className="text-center p-12">Loading questions for assessment...</div>;
    }
